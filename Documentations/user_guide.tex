...
\begin{itemize}
   ...
   \item Les derniers caractères contiennent plusieurs informations propres aux règles spécifiques des échecs (tels que le roque ou le coup du pion en passant) et ne seront pas détaillés dans ce guide. Pour plus de détails sur le système FEN, des liens sont disponibles en fin de guide 
\end{itemize}
...
Premièrement, il faut savoir si la ressource est une base de données de débuts de partie (openings database), une base de données de fins de partie (endings database) ou un moteur d'échec (bot).
