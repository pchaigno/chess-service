...
Si elle contient \$url_fen\$ alors la requête doit être GET, sinon elle doit être POST.
...
Pour faire appel à cette fonction, le framework utilise la fonction PHP \textit{call_user_func}.
...
De la même façon, si le serveur central contacte une ressource distante ou reçoit une réponse, le \textit{listener} \textit{ResourceListener} en sera notifié (voir figure \ref{fig:uml-listeners}).
Enfin, pour tous les évènements liés à l'interface RESTful du serveur central, c'est l'interface \textit{CentralServerListener} qui en sera notifiée (voir figure \ref{fig:uml-listeners}).
